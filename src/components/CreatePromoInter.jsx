...
import { createPromoInter } from "../redux/actions";
import { PromosInterBorrar } from "./PromosInterBorrar";
import {UserContext} from "../StreamerContext"
export const CreatePromoInter = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await dispatch(createPromoInter(promoData, user.username, user.password)); // Pasando las credenciales
...
<hr  className="border-0 border-t-[0.1rem] w-10/12 max-[963px]:w-8/12  border-white"/>
         <h1 className="text-center">INFO EXTRA</h1>
          ...
          <div className="flex gap-x-[1rem] w-full justify-center mb-[1rem]">
          <textarea  className='text-black w-full h-[15rem] '  placeholder="Texto descriptivo del casino" onChange={handleChange} value={promoData.infoparrafo} name="infoparrafo"  ></textarea>
          </div>
