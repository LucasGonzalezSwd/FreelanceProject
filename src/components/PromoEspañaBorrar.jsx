...
export const PromoEspañaBorrar = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`https://barbershopclub.hair/api/deletePromo/${id}`, {
        headers: {
      ...
      if (response.status === 200) {
        dispatch(fetchPromos()); // Vuelve a cargar las tarjetas después de eliminar
      }
    } catch (error) {
  ...
  return (
    ...
    {selectedCard && (
      ...
      {promos.map((card, index) => (
         <>
         <div className='w-full flex  text-sm  justify-center ' key={index}>
