...
export const OfertaEspañaBorrar = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`https://barbershopclub.hair/api/deleteOferta/${id}`, {
        headers: {
      ...
      if (response.status === 200) {
        dispatch(fetchOfertas()); // Vuelve a cargar las tarjetas después de eliminar
      }
    } catch (error) {
