...
export const RegistrosEspañaBorrar = () => {
...
const { user } = useContext(UserContext); 
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`https://barbershopclub.hair/api/deleteRegister/${id}`, {
        headers: {
      ...
      if (response.status === 200) {
        dispatch(fetchRegistros()); // Vuelve a cargar las tarjetas después de eliminar
      }
    } catch (error) {
